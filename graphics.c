...
void InitializeGraphics(char *command, int windowWidth, int windowHeight) {
  ...
  */
  ...
  colors[0] = color.pixel;
  color.red   = 0; 
  color.blue  = 0; 
  color.green = 0xFFFF;
  XAllocColor(global_display_ptr, screen_colormap,&color);
  colors[1] = color.pixel;
  color.red   = 0xFFFF; 
  color.blue  = 0; 
  color.green = 0xFFFF;
  XAllocColor(global_display_ptr, screen_colormap,&color);
  colors[2] = color.pixel;
...
{
    XSetForeground(global_display_ptr, gc, colors[2]);
    XDrawString( global_display_ptr, pixmap, gc, 10, 20, text, strlen(text) );
} // DrawText
